...
public class ZoomView extends FrameLayout {
    ...
    private boolean zoomAble;//是否能放大
    private boolean doubleClickAble;//是否双击放大
    ...
    public boolean dispatchTouchEvent(final MotionEvent ev) {
        ...
        if (!zoomAble) return super.dispatchTouchEvent(ev);
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ZoomView, defStyle, 0);

        zoom = a.getFloat(R.styleable.ZoomView_zoom, 1f);
        smoothZoom = a.getFloat(R.styleable.ZoomView_smoothZoom, 1f);
        maxZoom = a.getFloat(R.styleable.ZoomView_maxZoom, 5f);
        doubleClickAble = a.getBoolean(R.styleable.ZoomView_doubleClickAble, true);
        zoomAble = a.getBoolean(R.styleable.ZoomView_zoomAble, true);
    }
...
// tap
                if (l < 30.0f) {
// check double tap
                    if (doubleClickAble && System.currentTimeMillis() - lastTapTime < 500) {
                        if (smoothZoom == 1.0f) {
